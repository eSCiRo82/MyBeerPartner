...
/**
 * Use case to download the episodes from the API
 */
class DownloadEpisodesUseCase @Inject constructor(
    ...
    override fun launchFlow(input: Unit): Flow<Boolean> = flow {
        tailrec fun go(page: Int, result: RickAndMortyDataRepository.RequestResult) : RickAndMortyDataRepository.RequestResult =
            when (result) {
                RickAndMortyDataRepository.RequestResult.Available -> go(page + 1, repository.requestEpisodes(page))
                else -> result
            }
